...
import { TEMATY, WIEK, TRYBY, kanonicznyTemat, liczbaPytan } from './konfig.js?v=m12-46';
import { czyWspolrzedneOk, formatujWspolrzedne, odlegloscM } from './geo.js?v=m12-46';
...
/**
 * Warianty rev4/rev5 (decyzja właściciela 2026-09-09, B2): kod pozycyjny
 * `poprawna` ZOSTAJE, odwracanie tekstu ZNIKA.
 *
 * Powód: modele przekręcały wyrazy przy odwracaniu, więc bariera przypadkowego
 * wglądu kosztowała jakość pytań. Sam kod pozycyjny wystarcza do tego, po co
 * odwracanie powstało (ADR 0007: maskowanie, nie szyfrowanie) — gracz zaglądający
 * w JSON widzi treść, ale nie wie, która odpowiedź jest poprawna.
 *
 * Dwa markery, bo profil walidacji źródeł nadal się różni (ADR 0032):
 * rev4 = z fact-check (źródła twarde), rev5 = bez fact-check (źródła opcjonalne).
 * rev1/rev2/rev3 czytamy dalej — paczki leżące na Drive muszą działać.
 */
export const WERSJA_PROTOKOLU_REV4 = 'PYT/1.0-rev4';
export const WERSJA_PROTOKOLU_REV5 = 'PYT/1.0-rev5';

/** Warianty, w których `poprawna` jest kodem pozycyjnym (§3.4). */
export const WARIANTY_Z_KODEM = Object.freeze([
  WERSJA_PROTOKOLU_REV2, WERSJA_PROTOKOLU_REV3, WERSJA_PROTOKOLU_REV4, WERSJA_PROTOKOLU_REV5,
]);
...
export const SZABLON_WERSJA = 'PYT/1.0.7'; // 1.0.7: koniec odwracania liter, zostaje kod poprawnej (B2, 2026-09-09)

/** Wersja szablonu bez weryfikacji (§2.2) — wersjonowana niezależnie od §2. */
export const SZABLON_WERSJA_BEZ_WERYFIKACJI = 'PYT/1.0-nofc.2'; // .2: bez odwracania liter (B2)
...
export { SCHEMAT_KONTENERA, KODOWANIE } from './kodowanie.js?v=m12-46';
...
export function zbudujPrompt({ konfig, okolica, stacje, teraz = new Date(), factcheck = false }) {
  ...
  // Domyślnie wariant bez weryfikacji (ADR 0032): szybszy, z pamięci modelu.
...
export function odkodujPaczkeRev2(paczka) {
  ...
  // B2 (2026-09-09): rev4/rev5 mają tekst zapisany wprost — odwracamy pola
  // TYLKO dla starych paczek rev2/rev3, żeby dalej dawały się odczytać.
  const odwrocone = paczka.protokol === WERSJA_PROTOKOLU_REV2 || paczka.protokol === WERSJA_PROTOKOLU_REV3;
  const robocza = odwrocone ? odwrocPolaPaczki(paczka) : structuredClone(paczka);
...
export function poprawkaDlaModelu(usterki, { liczbaPytan, factcheck = true } = {}) {
  ...
  linie.push('', factcheck
    ...
    : 'Przypomnienie zasad twardych: fakty z własnej wiedzy (bez kwerendy w internecie), źródła opcjonalne (tylko pewne adresy albo pusta lista), brak zmyślonych nazw i dat, wszystkie pytania o tę samą okolicę.');
  return linie.join('\n');
}
