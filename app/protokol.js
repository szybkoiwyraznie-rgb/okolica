...
import { TEMATY, WIEK, TRYBY, KLUCZE_POZIOMOW, POZIOMY, POZIOM_DORMYSLNY, kanonicznyTemat } from './konfig.js?v=m12-163';
import { czyWspolrzedneOk, odlegloscM } from './geo.js?v=m12-163';
...
export const SZABLON_WERSJA = 'PYT/1.4.0'; // 1.4.0: bez współrzędnych w prompcie (środek gry + stacje) i okolica bez lat/lon w schemacie (ADR 0060); 1.3.1: krótki prompt — zestawienie pod stacjami, bez imion i daty, bez źródeł (ADR 0057/0058); 1.2.1: sekcja GRACZE — POZIOMY PYTAŃ (ADR 0055); 1.1.2: „tę samą liczbę pytań" (gramatyka)

/** Wersja szablonu bez weryfikacji (§2.2) — wersjonowana niezależnie od §2. */
export const SZABLON_WERSJA_BEZ_WERYFIKACJI = 'PYT/1.4-nofc.0'; // jak 1.4.0 — te same zdania w wariancie bez weryfikacji
...
export function opisListyStacji(stacje, srodek, { liniaPoziomow = '' } = {}) {
  return stacje
    .map((s, i) => {
      // ADR 0060: bez współrzędnych — model nie korzysta z liczb (właściciel
      // zmierzył ±20 km identyfikacji); kotwicą są nazwy własne z
      // OpenStreetMap (opis) + odległość od środka gry.
      ...
      const linia = `- stacja ${i + 1}: ${opis}${dystans != null ? ` (${dystans} m od środka gry)` : ''}`;
...
export function zbudujPrompt({ konfig, okolica, stacje, teraz = new Date(), factcheck = false, rodzajGry = 'hotseat', multiPoziom = POZIOM_DORMYSLNY } = {}) {
  ...
  const podstawienia = {
    // ADR 0060: współrzędnych w prompcie nie ma (model nie korzysta z liczb)
    MIEJSCE: okolica.miejsce && okolica.miejsce.trim() ? okolica.miejsce.trim() : 'brak odczytu nazwy miejsca',
...
export function walidujPaczke(paczka, oczekiwane = {}) {
  ...
  if (!okolica || typeof okolica !== 'object') {
    dodaj('E15', 'okolica', 'Brak obiektu "okolica" z promienM i miejsce.');
  } else {
    // ADR 0060: paczka nie niesie lat/lon — model nie zwraca już współrzędnych
    // (w prompcie ich nie ma). Stare paczki NIESIĄ te pola: są ignorowane,
    // tak jak wycofane pola w ADR 0058 (dodawczość).
