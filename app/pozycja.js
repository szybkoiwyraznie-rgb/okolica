...
import { bearingStopnie, czyDotarl, czyWspolrzedneOk, ogranicz, odlegloscM, przesunPunkt, progDojsciaM } from './geo.js';
...
export function ocenFix(fix, { maxAccuracyM = GRANICE.maxAccuracyM } = {}) {
  ...
  if (fix.accuracy == null || !Number.isFinite(fix.accuracy) || fix.accuracy <= 0) {
    return {
      ...
      akceptowany: true,
      kod: null,
      komunikat: 'Brak danych o dokładności — próg dojścia jest ustawiony najostrzej (100 m).',
...
export function stanDojscia(historia, stacja, { wymaganeTrafnienia = GRANICE.wymaganeTrafnienia } = {}) {
  ...
  if (!stacja || !czyWspolrzedneOk(stacja.lat, stacja.lon)) {
    return {
      ...
      kod: 'P06',
      komunikat: 'Brak poprawnych współrzędnych stacji — nie ma czego rozstrzygać. Odśwież układ stacji albo zgłoś dojście ręcznie.',
    };
  }
