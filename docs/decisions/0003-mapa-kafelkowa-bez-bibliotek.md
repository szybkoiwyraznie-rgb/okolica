...
## Aneks 2026-09-12 (m12-81) jest w archiwum (poza budżetem lektury)

Ścieżka operatorska podmiany szablonu kafelków bez wdrażania wersji —
dosłownie: `docs/decisions/archive/aneksy-0003-2026-09-12.md` (archiwizacja
2026-09-17b; AGENTS.md §0, LESSONS L62/L66). Obowiązuje: klucz `localStorage`
**`okolica:kafelki:url`** nadpisuje szablon `osm` (czyta go raz
`wczytajNadpisanieKafelkow()`, waliduje `walidujSzablonKafelkow()` i przekazuje
`ustawSzablonKafelkow()`), walidacja wymaga `https:` i wszystkich trzech
podstawień `{z}/{x}/{y}` i **odrzuca, nie rzuca** (błędny klucz zostawia adres
wbudowany), nadpisać wolno wyłącznie `osm`, a `app/mapa.js` zostaje czysty (bez
`localStorage`). W Informacjach zostają link „Zgłoś błąd na mapie" i adres
kontaktowy — wymóg polityki OSM; całość domyka `docs/ASSETS.md` §1/§5.
